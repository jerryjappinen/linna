...
export default (siteTitle, canonicalBaseUrl, disableScriptSanitization) => {
  return {

    // NOTE: see nuxt.config.js for the defaults
    head () {
      let title = siteTitle
      const meta = []
      const links = []
      ...
      // Page title
      if (pageTitle) {
        title = pageTitle + (!includes(pageTitle, title) ? ' – ' + title : '')
      ...
      // Canonical URL
      const canonicalUrl = canonicalBaseUrl + this.$route.path
      links.push({
        rel: 'canonical',
        href: canonicalUrl
      })
